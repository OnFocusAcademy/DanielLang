...
module.exports = makeModule(NAME, () => ({
  ...
  // pipe a value through a series of unary functions from R to L
  pipe: makeFunction((val, ...fns) => fns.reduce((v, fn) => fn(v), val), {
    name: `${NAME}.pipe`,
    variadic,
  }),
  // compose 2 unary functions R to L
  ...
  "js-eval": makeFunction(eval, {`${NAME}.js-eval`}),
  // throwing exceptions
  Exception,
  RuntimeException,
  fail: makeFunction(
